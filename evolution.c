	...
	for( iter=0; iter<max_iter && current_max_food <= max_food && num_cells_alive > 0; iter++ ) {
		...
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				...
				//printf("la celula de posicion   %i   esta en el la casilla   %i   y tiene comida   %lf" , posicion, posicion, food_to_share[i]);
			}
		} // End cell movements
		...
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				...
				posicion = (int)cells[i].pos_row * (int)cells[i].pos_col;

				//printf("posicion   %i   casilla   %i   y la casilla tiene comida   %lf   en la iteracion   %i" , posicion, posicion, food, iter);
				//printf("\n");
