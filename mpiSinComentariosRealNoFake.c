	...
	*/
	...
	for( iter=0; iter<max_iter && current_max_food <= max_food && num_cells_alive > 0; iter++ ) {
		...
		*/
		...
		printf("soy el rango   %i    y tengo este numero de celulas          %i",rank, my_num_cells);
		printf("\n");
		printf("\n");
		printf("\n");
		printf("\n");
		...
		for (i=0; i<my_num_cells; i++){
	            ...
	            if(posicionMyCells<my_begin || posicionMyCells>my_end){ //Comprobamos si la celula se ha movido de procesador
	            	...
	            	printf("yo envio esto              sabes que lo envio si lo sabes %i      rank         ",rank);
	            	printf("\n");
		...
		//int enviadosTotal = 0;
		//MPI_Allreduce(&enviados, &enviadosTotal, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

		
		int *numeroCelulas = (int *)malloc( sizeof(int) * (size_t)procs );
		MPI_Allgather(&enviados, 1, MPI_INT, numeroCelulas, 1, MPI_INT, MPI_COMM_WORLD);
        ...
        for(j=0; j<procs; j++){
            disp[j]=offset;
            enviadosTotal += numeroCelulas[j];
            offset += numeroCelulas[j];
        }

        Cell *numero_RecibosTotales = (Cell *)malloc(sizeof(Cell ) * (size_t)enviadosTotal);
		MPI_Allgatherv(my_cellsEnviadas,enviados,MPI_CELL_EXT,numero_RecibosTotales,numeroCelulas,disp,MPI_CELL_EXT,MPI_COMM_WORLD);

		//MPI_Alltoall(my_cellsEnviadas,enviados,MPI_CELL_EXT,numero_RecibosTotales,enviadosTotal,MPI_CELL_EXT,MPI_COMM_WORLD);

		//printf("\n");
		//printf("sssssssssssssssssssssssssssssssssssssssss               %i           %i", my_num_cells, rank);
		//printf("\n");
		//printf("enviados en total%i", enviadosTotal);
		printf("\n");
		for(i=0; i<enviadosTotal;i++){
			posicionMyCells = (int) numero_RecibosTotales[i].pos_row * columns + (int) numero_RecibosTotales[i].pos_col; //posicion nueva de la celula despues del movimiento
			if(posicionMyCells>=my_begin && posicionMyCells<=my_end){
				my_num_cells++;
			}
		}
		//printf("sssssssssssssssssssssssssssssssssssssssss               %i           %i", my_num_cells, rank);
		...
		//printf("%i",numero_RecibosTotales[0].age);
		...
		*/
		...
		Cell *celulasRecibidas = (Cell *)malloc(sizeof(Cell ) * (size_t)(my_num_cells - my_num_cellsTemp));
		...
		for(i=0; i<enviadosTotal;i++){
			posicionMyCells = (int) numero_RecibosTotales[i].pos_row * columns + (int) numero_RecibosTotales[i].pos_col; //posicion nueva de la celula despues del movimiento
			if(posicionMyCells>=my_begin && posicionMyCells<=my_end){
				celulasRecibidas[indiceTemp] = numero_RecibosTotales[i];
				indiceTemp++;
			}
		}
		...
		for(i=0; i<my_num_cells; i++){
			if(my_num_cellsTemp<=i){
				...
				//printf("la iteracion en la que se añade       %i", i);
				//printf("\n");
				indice++;
			}
		}
		...
		for (i=0; i<my_num_cells; i++) {
			if ( my_cells[i].alive ) {
				...
				if ( my_cells[i].age > 30 && my_cells[i].storage > 20 ) {
					//printf("que bueno que viniste aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
					printf("\n");
					int new = 0;
	            	...
	            	if(posicionMyNewCells<my_begin || posicionMyNewCells>my_end){ //Comprobamos si la celula se ha movido de procesador
	            		printf("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa%i", rank);
	            		printf("\n");
		...
		int *numeroCelulas2 = (int *)malloc( sizeof(int) * (size_t)procs );
		MPI_Allgather(&enviados, 1, MPI_INT, numeroCelulas2, 1, MPI_INT, MPI_COMM_WORLD);
        ...
        for(j=0; j<procs; j++){
            disp2[j]=offset;
            enviadosTotal += numeroCelulas2[j];
            offset += numeroCelulas2[j];
        }

        Cell *numero_RecibosTotales2 = (Cell *)malloc(sizeof(Cell ) * (size_t)enviadosTotal);

		MPI_Allgatherv(my_cellsEnviadas2,enviados,MPI_CELL_EXT,numero_RecibosTotales2,numeroCelulas2,disp2,MPI_CELL_EXT,MPI_COMM_WORLD);
		//MPI_Alltoall(my_cellsEnviadas,enviados,MPI_CELL_EXT,numero_RecibosTotales,enviadosTotal,MPI_CELL_EXT,MPI_COMM_WORLD);
		...
		Cell *celulasRecibidas2 = (Cell *)malloc(sizeof(Cell ) * (size_t)(my_num_cells - my_num_cellsTemp));
		...
		for(i=0; i<enviadosTotal;i++){
			posicionMyCells = (int) numero_RecibosTotales[i].pos_row * columns + (int) numero_RecibosTotales[i].pos_col; //posicion nueva de la celula despues del movimiento
			if(posicionMyCells>=my_begin && posicionMyCells<=my_end){
				celulasRecibidas2[indiceTemp] = numero_RecibosTotales2[i];
				indiceTemp++;
			}
		}
		...
		//printf("antes de que pete tengo tantas celulas     %i     y soy el puto precesador        %i",my_num_cells, rank);
		//printf("\n");
		for (i=0; i<my_num_cells; i++) {
				if ( my_cells[i].alive ) {
		...
		*/
		...
		printf("\n");
		printf("estas son el numero de celulas al final        %i          el rank                  %i", my_num_cells, rank);
		printf("\n");
		//printf("\n");
		//printf("\n");
