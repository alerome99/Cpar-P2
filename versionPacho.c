	...
	for( iter=0; iter<max_iter && current_max_food <= max_food && num_cells_alive > 0; iter++ ) {
		...
		*/
		...
		for (i=0; i<my_num_cells; i++) {
	            ...
	            if(posicionMyCells<my_begin || posicionMyCells>my_end){ //Comprobamos si la celula se ha movido de procesador
	            	...
	            	if(rank == 0  && iter == 859){
						...
						printf("posicion  %i  soy el rango 0 y envio al %i\n",posicionMyCellsEnMiCulture, rankTemp);
	            	}
	        		...
	        		if(rank == 0  && iter == 859){
	        			for(j=0; j<procs; j++){
	        				printf("posicion %i  envia  %i",j ,numero_Envios[j]);
	        				printf("\n");
	        			}
	            	}
		...
		MPI_Alltoall(numero_Envios, 1, MPI_INT, numero_Recibos, 1, MPI_INT, MPI_COMM_WORLD);
			...
			/*	if(iter==859){
			for(i=0;i<procs;i++){
				printf("posicion de la celula enviada%i   soy el rank   %i\n",numero_Recibos[i], rank);
			}
		}*/
        ...
        if(iter == 859){
        	for(j=0; j<procs; j++){
            	printf("recibo   %i  y soy el rank  %i\n",numero_Recibos[j], rank);
        	}
        	/*
        	for(j=0; j<procs; j++){
        	    printf("display de enviar%i   y rank    %i     y soy el rank  %i\n",dispEnviados[j], j, rank);
        	}
        }*/
		...
		if(iter==859){
			printf("hola soy el rank  %i y mi begin es %i", rank, my_begin);
			printf("\n");
		}
		...
		if(iter==859){
			printf("hola soy el rank  %i", rank);
			printf("\n");
		}
