 ...
 */
	...
	for( iter=0; iter<max_iter && current_max_food <= max_food && num_cells_alive > 0; iter++ ) {
		...
		//	printf("wdwdwdwd %lf",cells[i].storage);
		//	printf("\n");
		//}
		...
		for (i=0; i<num_new_sources; i++) {
			...
			posicion = row*columns+col;
			if(posicion >= my_begin && posicion <= my_end){
				posicion = posicion - my_begin;
				culture[posicion] = culture[posicion] + food;
			}
		}
		...
		*/
		...
		//for (j = 0; j < my_num_cells; j++){
			//if(rank==3){
				//if(iter==859){
					//posicionMyCellsEnMiCulture = (int) cells[j].pos_row * columns + (int) cells[j].pos_col;
					//if(posicionMyCellsEnMiCulture == 160400){
						//printf("posicion  %i  soy el rango   %i\n",posicionMyCellsEnMiCulture,rank);
					//}
			//}
		//}
		...
		for (j=0; j<my_num_cells; j++) { 		
	        if(/*iter==33  || iter==33){
				posicionMyCellsEnMiCulture = (int) cells[j].pos_row * columns + (int) cells[j].pos_col;
				printf("\n");
				printf("DESPUES DEL BUCLE posicionMyCellsEnMiCulture  %i  edad  %i  iteracion  %i  storage  %lf   rango   %i  numero de celulas  %i", posicionMyCellsEnMiCulture, cells[j].age, iter, cells[j].storage, rank, my_num_cells);
				printf("\n");
			}
		}*/
		...
		*/
		...
		for (i=my_num_cellsTemp; i<my_num_cells; i++){
			cells[i] = celulasRecibidas[indice];
			indice ++;
		}
		...
		for (i=0; i<my_num_cells; i++) {
				...
				posicionMyCellsEnMiCulture = (int) cells[i].pos_row * columns + (int) cells[i].pos_col;
				posicionMyCellsEnMiCulture -= my_begin;
				float food = culture[posicionMyCellsEnMiCulture];
		...
		if ( step_new_cells > 0 ) {
			//printf("my_num_cells:  %i   stepnewcells:  %i\n",my_num_cells,my_step_new_cells);
			cells = (Cell *)realloc( cells, sizeof(Cell) * ( my_num_cells + step_new_cells ) );
			//printf("iterrrrrrr: %i   step_new_cells%i\n",iter,my_step_new_cells);
			 //printf("ppppp: %i   my_num_cells %i  j: %i   iter: %i\n",(my_num_cells+my_step_new_cells),my_num_cells,my_step_new_cells,iter);

			for (j=0; j<step_new_cells; j++){
				// printf("antesss: %i   suma: %i   my_num_cells %i  j: %i  iter: %i\n",my_cells[0].age,(my_num_cells+j),my_num_cells,j,iter);

				cells[ my_num_cells + j ] = new_cells[ j ];
				//printf("new celllllllllllllls: %i newww: %i  step_new_cells %i  j: %i  iter: %i\n",my_cells[my_num_cells+j].age,new_cells[j].age,my_num_cells,j,iter);
				}
